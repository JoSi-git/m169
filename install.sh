...
if ! grep -q "moodle-up()" "$SHELL_RC"; then
    {
        ...
        echo "moodle-backup() { (cd \"$INSTALL_DIR\" && sudo ./tools/moodle-backup/moodle-backup.sh \"\$@\"); }"
        echo "moodle-restore() { (cd \"$INSTALL_DIR\" && sudo ./tools/moodle-backup/moodle-restore.sh \"\$@\"); }"
        echo "moodle-cronjob() { (cd \"$INSTALL_DIR\" && sudo ./tools/moodle-backup/moodle-cronjob.sh \"\$@\"); }"
